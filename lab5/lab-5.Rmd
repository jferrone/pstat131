...
-   Note that no proportion was specified in `initial_split()`. Check the help information for this function. What is the default specification? Check the dimensions of the training and test sets to verify.
    ...
    *The default specification is 3/4.*

    ```{r}
    dim(Hitters_train)[1]/dim(Hitters)[1]
    dim(Hitters_test)[1]/dim(Hitters)[1]
    ```
...
-   Compare and contrast the estimates for these two very different penalty values. What do you notice?
    ...
    *Higher* `penalty` *values seem to lead to much more extreme prediction estimates in this case and the values tend to move closer to zero as the* `penalty` *rises.*
...
-   What do you notice about the predicted values for these observations as the value of `penalty` changes?
    ...
    *As the* `penalty` *changes, the predicted values seem to rise and increase in size.*
...
-   How does `penalty_grid` change if you set `levels = 100`? `levels = 10`?
    ...
    ```{r}
    grid_regular(penalty(range = c(-5, 5)), levels = 10)
    grid_regular(penalty(range = c(-5, 5)), levels = 100)
    ```

    *The* `penalty_grid` *continues to move towards 0 as* `levels` *increases.*

-   Investigate the other `grid` functions in `tidymodels`. Try one of them out here. What do you notice?

    ```{r}
    grid_random(penalty(range = c(-5, 5)))
    ```

    *This seems to much more sporadic and randomized, as it should.*
...
-   Evaluate the performance of this model. Look at the parameter estimates for the training data. What does this tell you about the parameters?
    ...
    *This* $R^2$ *value tells us that this is not the most accurate model, but in the case of sports predictions it is likely that this value would actually be quite suitable and represents an acceptable model for this situation.*
...
-   Which model, ridge or lasso, performed best on the testing data?
    ...
    *In this case, there seems to be very little disparity between the ridge and lasso models. Their* $R^2$ *values are nearly equivalent, and thus neither is particularly more effective than the other.*
...
-   Instead of using `select_best()`, try selecting by one standard error, and refit to the training and testing sets. Does anything change?
    ...
    ```{r}
    best_penalty <- select_by_one_std_err(tune_res, metric = 'rsq', desc(penalty))
    ...
    *This shows a significant reduction in the estimate, and represents a worse model in the end.*
