...
- How many observations are there in `diamonds`?
    ...
    - *There are 53,940 observations in `diamonds`.*
    

- How many variables? Of these, how many are features we could use for predicting `price`?

    - *There are 10 variables including `price` which means that there are nine variables which could be used to predict `price`.*
...
diamonds %>% 
  select(is.numeric) %>% 
  cor() %>% 
  corrplot(type = 'lower',   # Lower Diagonal
           diag = FALSE,     # Diagonal Elements
           method = 'color') # Use Colors to Display Correlations
```
...
- Which features are positively correlated with `price`? Do these make sense?
    ...
    - *The features positively correlated with `price` include `carat`, `x`, `y`, and `z`.* 

- Are any features negatively correlated with `price`?

    - *There are no features negatively correlated with price.*
...
- What do you learn from this plot about the relationship between `price`, `cut`, and `color`?
    ...
    - *This plot reveals quite a bit about the relationship between `price`, `cut`, and `color`. The `price` of a diamond does not seem to necessarily indicate a good `cut`, rather the `color` seems to be a slightly more important indicator of `price`.*
    

- Refer back to the definitions of the variables in `?diamonds`. Does anything you learned surprise you?

    - *What each value of `color` represents is pretty surprising, as `J` is typically more expensive. It is possible that this is due to a larger `carat` or size. Additionally, the cheapest `cut` seems to be the best `cut`, `ideal`.*
...
- Explain why lower-quality colors tend to cost more.
    ...
    - *Lower-quality colors tend to cost more as they are related to a larger `carat`, which is a bigger size.*
...
- What do you think the `strata = price` argument does? Take a guess, then use `?initial_split` to verify.
    ...
    - *`strata = price` sets the output variable.*
    - *The variable used to conduct stratified sampling.*
...
- Use the Internet to find documentation about the possible `step` functions. Name three `step` functions that weren't used here and describe what they do.
    ...
    - *`step_date()` converts date data into factor or numeric variable(s)*^^1
    - *`step_regex()` creates a new dummy variable based on a regular expression*^^2
    - *`step_count()` creates a variable that counts instances of a regular expression patterin in text.^^3
    
    1. https://recipes.tidymodels.org/reference/step_date.html
    2. https://www.rdocumentation.org/packages/recipes/versions/0.1.5/topics/step_regex
    3. https://www.rdocumentation.org/packages/recipes/versions/0.1.17/topics/step_count
...
- Explain what the intercept represents. 
    ...
    - *The intercept represents the mean value if all the predictors were equal to zero.*
...
- Describe the effect of `carat`. Is it a significant predictor of `price`? Holding everything else constant, what is the effect on `price` of a one-unit increase in `carat`?
    ...
    - *The most significant predictor of `price` is `carat`. If everything is held constant, a one-unit increase in `carat` increases price by 11,281.55*
...
```{r}
diamond_train_res <- bind_cols(
  diamond_train_res, diamonds_train %>% select(price)
  )
diamond_train_res %>% 
  head()
```
