...
# Arrived more than two hours late, but didn't leave late
flights %>% filter(arr_delay >= 120 & dep_delay <= 1)
...
```{r}
flights %>% select(origin, dest, dest)
```
...
flights %>% mutate(
  ...
  sched_dep_time = (sched_dep_time %/% 100 * 60 + dep_time %% 100) %% 1440
  )
```
...
This returns a series of numbers which follow the following pattern:
