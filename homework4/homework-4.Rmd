...
collect_metrics(log_fit, summarize=FALSE)

# Log Metrics Summarized
collect_metrics(log_fit)

# LDA Metrics per Fold
collect_metrics(lda_fit, summarize=FALSE)

# LDA Metrics Summarized
collect_metrics(lda_fit)

# QDA Metrics per Fold
collect_metrics(qda_fit, summarize=FALSE)

# QDA Metrics Summarized
...
```{r}
log_acc <- predict(log_test, new_data = titanic_test, type='class') %>%
  bind_cols(titanic_test %>% select(survived)) %>%
  accuracy(truth = survived, estimate = .pred_class)
lda_acc <- predict(lda_test, new_data = titanic_test, type='class') %>%
  bind_cols(titanic_test %>% select(survived)) %>%
  accuracy(truth = survived, estimate = .pred_class)
qda_acc <- predict(qda_test, new_data = titanic_test, type='class') %>%
  bind_cols(titanic_test %>% select(survived)) %>%
  accuracy(truth = survived, estimate = .pred_class)

results <- bind_rows(log_acc, lda_acc, qda_acc) %>%
  tibble() %>% mutate(model = c('Log Model', 'LDA Model', 'QDA Model')) %>%
