...
*The histogram above displays the distribution of `age` which is primarily clustered between the values of 8 and 15. Outside of these range there are far fewer values on either tail.*
...
```{r}
abalone_recipe <- abalone_recipe %>% step_center(all_numeric_predictors())
```

4.  scale all predictors.

```{r}
abalone_recipe <- abalone_recipe %>% step_scale(all_numeric_predictors())
```
...
```{r}
lm_wflow <- workflow() %>%    #1
  add_model(lm_model) %>%     #2
  add_recipe(abalone_recipe)  #3
```
...
cur_test <- data.frame(type = 'F', rings=NA, longest_shell = 0.50, diameter = 0.10, height = 0.30, whole_weight = 4, shucked_weight = 1, viscera_weight = 2, shell_weight = 1)

predict(lm_fit, cur_test)
```
