...
-   What do you notice from this correlation matrix? **Only year and volume seem to be correlated.**
-   Do you need to worry about predictors being highly linearly correlated with each other? **No, it is not likely that I need to worry about predictors being highly linearly correlated with each other, as there is no clear correlation.**
-   Does this matrix tell you anything about relationships between predictors and the outcome? **No, it does not.**
...
-   Describe the relationship between `Year` and `Volume`. There is a slightly positive relationship between `Year` and `Volume`, as `Volume` seems to rise after each `Year`. Though, this is not a significant relationship and the short time span is not truly enough to show that this correlation truly exists.
...
-   Are any of the predictors statistically significant? **No, as the p-value is much larger than 0.05.**
...
-   What do you notice from the confusion matrix?
    ...
    *The model seems to predict incorrectly about half of the time.*

-   Does the model tend to overpredict one class or the other? If so, which one?

    *The model seems to over-predict up.*

-   Do you think the model is doing well? Why or why not?

    *No, the model does not seem to be performing well as it is only correct about half of the time based on the confusion matrix.*
...
-   Compare the results of the LDA model to the results of the logistic regression model.
    ...
    ```{r}
    lda_acc$.estimate - log_reg_acc$.estimate
    ```

    *There is very little difference between the accuracy of these two models.*
...
-   What do the sensitivity and specificity values mean? Interpret them in terms of the concepts and data.
    ...
    *The **sensitivity** is the percentage of upwards mobility which was correctly predicted, while the **specificity** is the percentage of downwards mobility which is correctly predicted.*

-   How well did the model perform on the **testing** data?

    *It performed slightly worse on the testing data, at a level 2% lower.*

-   Why do you think the model performed like it did?

    *It is extremely hard to predict the stock market, and too many unquantifiable events occur which can affect the outcome each day. I.e. there is not enough data available to accurately predict the stock market.*
...
-   Do you think we could have done the initial split differently? Is there anything about the structure of the data that could inform the training/testing split?
    ...
    *The split could be done in such a way that it is over time. Stock data is ordered to some degree and the day to day events are not taken into account in this example.*
