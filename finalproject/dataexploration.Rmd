...
```{r}
ggplot(filter(school_df, attendance_rate > 0.7), aes(x = attendance_rate, y = avg_proficiency)) + geom_point
...
school_recipe <- recipe(avg_proficiency ~ ., data=school_train) %>%
  ...
  step_rm(c('math_proficiency', 'ela_proficiency')) %>%
  step_impute_linear(school_income_estimate, 
                     impute_with=imp_vars(economic_need_index))
...
school_metrics(school_train_res, truth=avg_proficiency, estimate=.pred)
```

## Testing Metrics
```{r}
school_test_metrics <- metric_set(rmse, rsq, mae)
school_test_metrics(school_test_res, truth=avg_proficiency, estimate=.pred)
```
